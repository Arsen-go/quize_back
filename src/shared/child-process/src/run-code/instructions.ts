...
const commandMap = (jobID: string, language: string) => {
  switch (language) {
    case 'java':
      return {
    ...
    case 'ruby': // TODO wip
      return {
        executeCodeCommand: 'ruby',
        executionArgs: [join(process.cwd(), `codes/${jobID}.rb`)],
        compilerInfoCommand: 'ruby --version',
      };
    default:
...
const supportedLanguages = ['java', 'cpp', 'py', 'c', 'js', 'go', 'cs', 'ruby'];

export { commandMap, supportedLanguages };
